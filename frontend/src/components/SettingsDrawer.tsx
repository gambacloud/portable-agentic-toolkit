      ...
      <div className="p-4 space-y-5">
        ...
        <div>
          <div className="flex items-center justify-between mb-2">
            <label className="text-xs font-medium text-gray-400">Knowledge Base</label>
            <a
              href="/kb-ui"
              target="_blank"
              rel="noreferrer"
          ...
          <KbDropdown
            docs={ragDocs}
            selected={settings.kbSources}
            onToggle={(source) => {
              const next = settings.kbSources.includes(source)
                ? settings.kbSources.filter((s) => s !== source)
                : [...settings.kbSources, source];
              onChange({ kbSources: next });
            }}
          />
        </div>
      ...
      >
        <span className="text-xs text-gray-300">
          {docs.length === 0
            ...
            : selected.length === 0
            ? "None selected"
            : `${selected.length} of ${docs.length} active`}
        </span>
        <svg
          viewBox="0 0 24 24"
          fill="none"
          ...
          <div className="max-h-52 overflow-y-auto py-1">
            {docs.map((doc) => (
              ...
              >
                <input
                  type="checkbox"
                  checked={selected.includes(doc.source)}
                  onChange={() => onToggle(doc.source)}
                  className="w-3.5 h-3.5 accent-indigo-500 shrink-0"
                />
